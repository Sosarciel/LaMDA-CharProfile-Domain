        ...
        for(const sceneName in sceneTextTable){
            ...
            if(scene!=null){
                if(scene.dialog && scene.dialog.length <=0){
                    SLogger.warn(`CahrConfig.loadScenes 错误 dialog长度小于等于0 sceneName:${sceneName} scene:${scene}`);
                    continue;
                }
                if(scene.dialog && scene.dialog[scene.dialog.length-1].sender_name!=this.getCharDisplayName()){
                    SLogger.warn(`CahrConfig.loadScenes 错误 dialog不以角色回复结尾 sceneName:${sceneName} scene:${scene}`);
                    continue;
                }
